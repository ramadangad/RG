    ...
    ks_renderGantt: function () {
      ...
      gantt.templates.tooltip_text = function (start, end, task) {
        ...
        if (
          ...
          if (gantt.config.ks_project_tooltip_config.ks_tooltip_task_actual_progress) {
            ks_tooltip_text +=
              "<b>" +
              _t("actual_Progress : ") +
              "</b> " +
...
//                var ks_new_project_id = this.state.groupedBy[ks_groupb_date].split(':')[0]
    ...
    ks_data_update: function (
      ...
      var ks_gantt_data = {
        ...
        actual_progress: each_task[ks_gantt_fields.ks_task_actual_progress] / 100,
        ks_actual_progress: each_task[ks_gantt_fields.ks_task_actual_progress],
...
//       Check if parent task is available then set add parent otherwise set to false.
    ...
    ks_project_task_data_update: function (
      ...
      var ks_gantt_datas = {
        ...
        actual_progress: each_project_task.actual_progress / 100,
        ks_actual_progress: each_project_task.actual_progress,
    ...
    ks_task_dynamic_content: function () {
          ...
          var textWidth =
            ((task.text || "").length +
              (gantt.config.ks_task_dynamic_progress
                ? (" (" + Math.round(task.progress * 100) + "%)" + "Actual: " + Math.round(task.actual_progress * 100) +
                 "%)
                ").length
                : 0)) *
      ...
      gantt.templates.task_text = function taskTextTemplate(start, end, task) {
        ...
        if (
          ...
          ks_task_text += " (Actual: " + Math.round(task.actual_progress * 100) + "%)";
        }
        ...
        gantt.eachTask(function (task) {
          ...
          while (currDate < task.end_date) {
            ...
            if (date <= today) {
              ...
              timegrid[timestamp].real += 1 * (task.actual_progress || 0);
            }
      ...
      let ks_gantt_config_columns = [
        {
          ...
          template: function (ks_task) {
            if (["task", "milestone"].indexOf(ks_task.type) > -1) {
              ...
              if (ks_task.ks_actual_progress == 0) {
                // Red
                return "<div class='ks_progress_sec ks_progress_red'></div>";
              } else if (ks_task.ks_actual_progress > 0 && ks_task.ks_actual_progress < 100) {
                // Yellow
                return "<div class='ks_actual_progress_sec ks_actual_progress_yellow'></div>";
              } else if (ks_task.ks_actual_progress == 100) {
                // green.
                return "<div class='ks_progress_sec ks_progress_green'></div>";
              }
            }
...
//          els.appendChild(task_duration_el);
        ...
        if (
          ...
          if (gantt.config.ks_task_dynamic_progress) {
            el.textContent += " (" + Math.round(task.progress * 100) + "%)" + "Actual:" + Math.round(task
            .actual_progress *
            100) + "%)";
          }
